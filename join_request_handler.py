...
Auto-approves join requests for managed groups (Async)
"""
import logging
...
class JoinRequestHandler:
    """Handles chat join requests (Async)"""
    ...
    async def process_join_request(self, chat_id: int, user_id: int, username: str = None, first_name: str = 'Unknown', last_name: str = None):
        ...
        try:
            ...
            group = await self.db.get_managed_group(chat_id)
            if not group:
            ...
            if await self.db.user_in_managed_group(user_id):
                logger.warning(f"User {user_id} already in a TGMS managed group, rejecting join request")
                await self.db.insert_join_request(user_id, chat_id, username)
                try:
                    await self.db.update_join_request_status_by_user_chat(user_id, chat_id, 'rejected_already_member')
                except Exception:
            ...
            # Auto-approve
            response = await self.api.approve_join_request(chat_id, user_id)
            
            if response.get("ok"):
                logger.info(f"✓ Approved join request: user {user_id} ({username}) → group {chat_id}")
                
                # Update user's group membership
                await self.db.update_user_group(user_id, chat_id, first_name, last_name, username)
                
                # Update request status in database
                try:
                    await self.db.update_join_request_status_by_user_chat(user_id, chat_id, 'approved')
                except Exception:
                    pass
                return True
            else:
                ...
                try:
                    await self.db.update_join_request_status_by_user_chat(user_id, chat_id, 'failed')
                except Exception:
                    pass
                return False
